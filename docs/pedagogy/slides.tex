...
\begin{frame}{Layers, in build order}
  ...
  \begin{enumerate}
    ...
    \item \texttt{gui/} --- a view over everything above; computes nothing new
  \end{enumerate}
\end{frame}
...
\begin{frame}{\texttt{gui/}: one state, three tabs}
  \revsec{4.16}
  \begin{itemize}
    \item One \texttt{CatalogState} object shared by Catalog, Poles, Strips --- so the tabs cannot disagree with each other
    \item Computes \textbf{no science}: every number shown is something the command line already wrote or could write
    \item \texttt{to\_json}/\texttt{from\_json}: a session is a small, shareable, human-diffable file
  \end{itemize}
\end{frame}

\begin{frame}{\texttt{gui/}: lazy stacks \& rasterisation}
  \revsec{4.16}
  \begin{itemize}
    \item A frame stack can be 2.25~GB --- \texttt{open\_stack} never calls \texttt{.load()}; one time step read per player move
    \item Measured: 280~MB resident, $<1$~s to open; $\sim$0.2~s per subsequent step
    \item Catalog map: \texttt{datashader} rasterisation for \emph{all} 47{,}600 points; individual hover points added only below 5{,}000 filtered rows
  \end{itemize}
\end{frame}

\begin{frame}{\texttt{gui/}: the graticule seam}
  \revsec{4.16}
  Contouring raw longitude draws a spurious line straight across the map at the 359$^\circ\to$0$^\circ$ wrap.
  \begin{block}{Fix: contour a wrapped difference, not the raw field}
    $((\text{lon} - \text{level} + P/2) \bmod P) - P/2$, masked beyond a quarter period, then contour its \emph{zero} crossing.
  \end{block}
  One helper, reused for latitude, longitude, \emph{and} local-time-of-day contours (period 24~h).
\end{frame}
