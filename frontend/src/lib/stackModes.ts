...
export const LEVEL_BLURBS: Record<StackLevel, string> = {
  ...
  frame: 'Instrument frames: one step per raw JIRAM frame, in the order the imager recorded them.',
};
...
export function parseStem(stem: string): { band: string; orbits: string; level: StackLevel } | null {
  const match = /^([A-Za-z]+)_orbits(.+)_([A-Za-z]+)$/.exec(stem);
  if (!match) return null;
  const level = match[3].toLowerCase();
  if (!isStackLevel(level)) return null;
  return { band: match[1].toUpperCase(), orbits: match[2].toLowerCase(), level };
}
